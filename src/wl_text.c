...
void TimedPicCommand(void)
{
    ...
    //
    TimeCount = 0;
    while (TimeCount < (longword)picdelay)
        ;
...
void HandleCommand(void)
{
    ...
    case 'C':       // ^c<hex digit> changes text color
        ...
        i = toupper(*++text);
        if (i >= '0' && i <= '9')
            fontcolor = (byte)(fontcolor + (i - '0'));
        else if (i >= 'A' && i <= 'F')
            fontcolor = (byte)(fontcolor + (i - 'A' + 10));
        text++;
        break;
...
void NewLine(void)
{
    ...
    if (++rowon == TEXTROWS)
    {
        ...
        if (*text == '^')
        {
                ch = (char)toupper((unsigned char)*(text + 1));
                if (ch == 'E' || ch == 'P')
                {
                    layoutdone = true;
                    return;
...
void HandleWord(void)
{
    ...
    while (*text > 32)
    {
        wordtxt[wordindex] = *text++;
        if (++wordindex == WORDLIMIT)
            Quit("PageLayout: Word limit exceeded");
    }
    wordtxt[wordindex] = 0;     // stick a null at end for C

    //
    // see if it fits on this line
    //
    VW_MeasurePropString(wordtxt, &wwidth, &wheight);

    while ((int)px + (int)wwidth > (int)rightmargin[rowon])
    {
        NewLine();
        if (layoutdone)
    ...
    // print it
    //
    newpos = (word)(px + wwidth);
    VWB_DrawPropString(wordtxt);
    px = newpos;
...
void PageLayout(boolean shownumber)
{
    ...
    if (*text != '^' || (char)toupper((unsigned char)*++text) != 'P')
...
void CacheLayoutGraphics(void)
{
    ...
    do
    {
        if (*text == '^')
        {
            ch = (char)toupper((unsigned char)*++text);
