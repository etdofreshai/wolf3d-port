...
Uint32		demo_last_ticks;
boolean		demo_debug_enabled;
int			demo_debug_frame;
...
void PollControls (void)
{
	int		max,min,i;
	byte	buttonbits;
	static boolean demo_debug_inited;
	static int poll_debug_frames = 0;
...
//
	...
	{
		if (!demo_debug_inited)
		{
			demo_debug_inited = true;
			demo_debug_enabled = SDL_getenv("WOLF3D_DEBUG_DEMO") != NULL;
			demo_debug_frame = 0;
		}
	...
	if (poll_debug_frames < 8) {
		fprintf(stderr, "[poll-frame][%d] demoplayback=%d demorecord=%d tics=%u\n",
			poll_debug_frames, (int)demoplayback, (int)demorecord, (unsigned)tics);
		fflush(stderr);
		++poll_debug_frames;
	}
	...
	//
		...
		controlx = *demoptr++;
		controly = *demoptr++;
		if (demo_debug_enabled && demo_debug_frame < 16)
		{
			fprintf(stderr, "[demo-input][%d] bits=%u x=%d y=%d tics=%u\n",
				demo_debug_frame, buttonstate[0] << 7 | buttonstate[1] << 6 | buttonstate[2] << 5 | buttonstate[3] << 4 |
				buttonstate[4] << 3 | buttonstate[5] << 2 | buttonstate[6] << 1 | buttonstate[7], controlx, controly, tics);
			fflush(stderr);
		}
...
//
	...
	//
		...
		*demoptr++ = controly;
		...
		if (demo_debug_enabled && demo_debug_frame < 16)
		{
			fprintf(stderr, "[demo-input][%d] consumed=%d bytes, pos=%ld\n",
				demo_debug_frame, tics, (long)(demoptr - ((char *)demobuffer)));
			demo_debug_frame++;
		}
	}
}
...
void PlayLoop (void)
{
	static int playloop_hit_count = 0;
	if (playloop_hit_count < 8) {
		fprintf(stderr, "[playloop] entering playloop demoplayback=%d demorecord=%d startgame=%d\n",
			(int)demoplayback, (int)demorecord, (int)startgame);
		fflush(stderr);
		playloop_hit_count++;
	}
