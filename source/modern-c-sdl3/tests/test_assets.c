...
static int check_wl6(const char *dir) {
    ...
    size_t wl6_model_build_attempts = 0;
    for (size_t map_index = 0; map_index < 60; ++map_index) {
        ...
        CHECK(wl_read_map_plane(gamemaps_path, &wl6_map, 1, mh.rlew_tag,
                                info_plane, WL_MAP_PLANE_WORDS) == 0);
        for (wl_difficulty difficulty = WL_DIFFICULTY_BABY;
             difficulty <= WL_DIFFICULTY_HARD;
             difficulty = (wl_difficulty)(difficulty + 1)) {
            ++wl6_model_build_attempts;
    ...
    CHECK(wl6_model_build_attempts == 240);
    CHECK(wl6_model_build_failures == 8);
    ...
    CHECK(wake_result.actor_tile_x == 5);
    CHECK(wake_result.actor_tile_y == 5);
    CHECK(wake_result.player_tile_x == 8);
    CHECK(wake_result.player_tile_y == 4);
    ...
    memset(&path_model, 0, sizeof(path_model));
    path_model.actor_count = 5;
    path_model.actors[0].kind = WL_ACTOR_GUARD;
    path_model.actors[0].mode = WL_ACTOR_STAND;
    path_model.actors[0].dir = WL_DIR_WEST;
    path_model.actors[0].tile_x = 5;
    path_model.actors[0].tile_y = 5;
    path_model.actors[0].patrol_remainder = 0x3000u;
    path_model.actors[0].shootable = 1;
    path_model.actors[1].kind = WL_ACTOR_OFFICER;
    path_model.actors[1].mode = WL_ACTOR_PATROL;
    path_model.actors[1].dir = WL_DIR_SOUTH;
    path_model.actors[1].tile_x = 6;
    path_model.actors[1].tile_y = 5;
    path_model.actors[1].shootable = 1;
    path_model.actors[1].ambush = 1;
    path_model.actors[2].kind = WL_ACTOR_DOG;
    path_model.actors[2].mode = WL_ACTOR_CHASE;
    path_model.actors[2].dir = WL_DIR_EAST;
    path_model.actors[2].tile_x = 7;
    path_model.actors[2].tile_y = 5;
    path_model.actors[2].shootable = 1;
    path_model.actors[3].kind = WL_ACTOR_DEAD_GUARD;
    path_model.actors[3].mode = WL_ACTOR_INERT;
    path_model.actors[3].tile_x = 8;
    path_model.actors[3].tile_y = 5;
    path_model.actors[3].shootable = 0;
    path_model.actors[4].kind = WL_ACTOR_GUARD;
    path_model.actors[4].mode = WL_ACTOR_STAND;
    path_model.actors[4].tile_x = WL_MAP_SIDE;
    path_model.actors[4].tile_y = 5;
    path_model.actors[4].shootable = 1;
    wl_actor_wake_all_result wake_all;
    CHECK(wl_wake_actors_for_chase(&path_model, 8, 4, 0, 1, &wake_all) == 0);
    CHECK(wake_all.actors_considered == 2);
    ...
    CHECK(path_model.actors[1].ambush == 1);
    CHECK(path_model.actors[2].mode == WL_ACTOR_CHASE);
    ...
    live_tick_pickup_model.statics[0].x = 4;
    live_tick_pickup_model.statics[0].y = 4;
    live_tick_pickup_model.statics[0].type = 50;
    live_tick_pickup_model.statics[0].bonus = 1;
    live_tick_pickup_model.statics[0].active = 1;
    CHECK(wl_init_player_gameplay_state(&pickup_state, 100, 3, 0, WL_EXTRA_POINTS) == 0);
    pickup_state.ammo = 80;
    ...
    live_tick_pickup_model.statics[0].x = 4;
    live_tick_pickup_model.statics[0].y = 4;
    live_tick_pickup_model.statics[0].type = 52;
    live_tick_pickup_model.statics[0].bonus = 1;
    live_tick_pickup_model.statics[0].active = 1;
    CHECK(wl_init_player_gameplay_state(&pickup_state, 100, 3, 0, WL_EXTRA_POINTS) == 0);
    live_tick_motion.x = 0x3f000u;
    ...
    for (size_t i = 0; i < sizeof(model_maps) / sizeof(model_maps[0]); ++i) {
        ...
        wl_runtime_tile_summary tile_summary;
        memset(&tile_summary, 0xff, sizeof(tile_summary));
        CHECK(wl_summarize_runtime_tiles(&model, &tile_summary) == 0);
        CHECK(wl_summarize_runtime_tiles(NULL, &tile_summary) == -1);
        CHECK(wl_summarize_runtime_tiles(&model, NULL) == -1);
        CHECK(tile_summary.clear_floor_count + tile_summary.solid_wall_count +
                  tile_summary.door_marker_count + tile_summary.moving_pushwall_marker_count +
                  tile_summary.other_marker_count == WL_MAP_SIDE * WL_MAP_SIDE);
        CHECK(tile_summary.clear_floor_count > 0);
        CHECK(tile_summary.solid_wall_count > 0);
        CHECK(tile_summary.door_marker_count > 0);
        CHECK(tile_summary.first_solid_wall_tile >= 1);
        CHECK(tile_summary.first_solid_wall_tile <= tile_summary.last_solid_wall_tile);

        wl_game_model synthetic_tiles;
        memset(&synthetic_tiles, 0, sizeof(synthetic_tiles));
        synthetic_tiles.tilemap[(0u)] = 1;
        synthetic_tiles.tilemap[(1u)] = 63;
        synthetic_tiles.tilemap[(2u)] = 0x80;
        synthetic_tiles.tilemap[(3u)] = 0xc0 | 37u;
        synthetic_tiles.tilemap[(4u)] = 90;
        wl_runtime_tile_summary synthetic_tile_summary;
        CHECK(wl_summarize_runtime_tiles(&synthetic_tiles, &synthetic_tile_summary) == 0);
        CHECK(synthetic_tile_summary.clear_floor_count == WL_MAP_SIDE * WL_MAP_SIDE - 5u);
        CHECK(synthetic_tile_summary.solid_wall_count == 2);
        CHECK(synthetic_tile_summary.door_marker_count == 1);
        CHECK(synthetic_tile_summary.moving_pushwall_marker_count == 1);
        CHECK(synthetic_tile_summary.other_marker_count == 1);
        ...
        wl_runtime_player_facing_run_summary facing_run;
        memset(&facing_run, 0xff, sizeof(facing_run));
        CHECK(wl_summarize_runtime_player_facing_run(&model, &facing_run) == 0);
        CHECK(wl_summarize_runtime_player_facing_run(NULL, &facing_run) == -1);
        CHECK(wl_summarize_runtime_player_facing_run(&model, NULL) == -1);
        CHECK(facing_run.direction == model.player.dir);
        CHECK(facing_run.has_blocking_tile == 1u || facing_run.hit_map_edge == 1u);
        CHECK(facing_run.traversed_tile_count >= facing_run.clear_floor_count);
        CHECK((size_t)facing_run.solid_wall + facing_run.door_marker +
                  facing_run.pushwall_marker + facing_run.other_marker <=
              1u);
        ...
        synthetic_player_neighborhood.player.y = 1;
        synthetic_player_neighborhood.player.dir = WL_DIR_EAST;
        synthetic_player_neighborhood.tilemap[(1u * WL_MAP_SIDE) + 3u] = 0x80u;
        CHECK(wl_summarize_runtime_player_facing_run(
                  &synthetic_player_neighborhood, &facing_run) == 0);
        CHECK(facing_run.direction == WL_DIR_EAST);
        CHECK(facing_run.clear_floor_count == 1u);
        CHECK(facing_run.traversed_tile_count == 2u);
        CHECK(facing_run.has_blocking_tile == 1u);
        CHECK(facing_run.hit_map_edge == 0u);
        CHECK(facing_run.first_blocking_tile == 0x80u);
        CHECK(facing_run.first_blocking_x == 3u);
        CHECK(facing_run.first_blocking_y == 1u);
        CHECK(facing_run.door_marker == 1u);
        synthetic_player_neighborhood.tilemap[(1u * WL_MAP_SIDE) + 3u] = 0u;
        CHECK(wl_summarize_runtime_player_facing_run(
                  &synthetic_player_neighborhood, &facing_run) == 0);
        CHECK(facing_run.has_blocking_tile == 0u);
        CHECK(facing_run.hit_map_edge == 1u);
        CHECK(facing_run.clear_floor_count == WL_MAP_SIDE - 2u);
        CHECK(facing_run.traversed_tile_count == WL_MAP_SIDE - 2u);
        synthetic_player_neighborhood.player.dir = WL_DIR_NONE;
        CHECK(wl_summarize_runtime_player_facing_run(
                  &synthetic_player_neighborhood, &facing_run) == 0);
        CHECK(facing_run.traversed_tile_count == 0u);
        CHECK(facing_run.hit_map_edge == 0u);
        synthetic_player_neighborhood.player.dir = (wl_direction)99;
        CHECK(wl_summarize_runtime_player_facing_run(
    ...
    CHECK(synthetic_engagements.chasing_threat_count == 2);
    CHECK(synthetic_engagements.ambush_threat_count == 1);
    ...
    chase_summary_model.actors[0].tile_x = 1;
    chase_summary_model.actors[0].tile_y = 1;
    chase_summary_model.actors[0].mode = WL_ACTOR_STAND;
    chase_summary_model.actors[1].tile_x = 2;
    ...
    chase_summary_model.actors[3].mode = WL_ACTOR_INERT;
    chase_summary_model.actors[4].tile_x = 5;
    chase_summary_model.actors[4].tile_y = 1;
    chase_summary_model.actors[4].mode = WL_ACTOR_BOSS_MODE;
    ...
    chase_summary_model.actors[6].tile_x = 7;
    chase_summary_model.actors[6].tile_y = 1;
    chase_summary_model.actors[6].mode = (wl_actor_mode)99;
    chase_summary_model.actors[7].tile_x = WL_MAP_SIDE;
    chase_summary_model.actors[7].tile_y = 1;
    chase_summary_model.actors[7].mode = WL_ACTOR_CHASE;
    wl_actor_mode_summary synthetic_modes;
    CHECK(wl_summarize_actor_modes(&chase_summary_model, &synthetic_modes) == 0);
    CHECK(wl_summarize_actor_modes(NULL, &synthetic_modes) == -1);
    CHECK(wl_summarize_actor_modes(&chase_summary_model, NULL) == -1);
    CHECK(synthetic_modes.stand_count == 1);
    CHECK(synthetic_modes.patrol_count == 1);
    CHECK(synthetic_modes.chase_count == 1);
    CHECK(synthetic_modes.inert_count == 1);
    CHECK(synthetic_modes.boss_mode_count == 1);
    CHECK(synthetic_modes.ghost_mode_count == 1);
    CHECK(synthetic_modes.invalid_mode_count == 1);
    CHECK(synthetic_modes.invalid_position_count == 1);
    ...
    chase_summary_model.statics[0].y = 10;
    chase_summary_model.statics[0].active = 1;
    chase_summary_model.statics[1].x = 14;
    chase_summary_model.statics[1].y = 10;
    chase_summary_model.statics[1].active = 1;
    chase_summary_model.statics[2].x = 10;
    chase_summary_model.statics[2].y = 14;
    chase_summary_model.statics[2].active = 1;
    chase_summary_model.statics[3].x = 6;
    chase_summary_model.statics[3].y = 10;
    chase_summary_model.statics[3].active = 1;
    chase_summary_model.statics[4].x = 13;
    chase_summary_model.statics[4].y = 13;
    chase_summary_model.statics[4].active = 1;
    chase_summary_model.statics[5].x = 10;
    chase_summary_model.statics[5].y = 6;
    chase_summary_model.statics[5].active = 0;
    chase_summary_model.statics[6].x = WL_MAP_SIDE;
    chase_summary_model.statics[6].y = 1;
    chase_summary_model.statics[6].active = 1;
    ...
    wl_static_line_of_sight_summary synthetic_static_los;
    CHECK(wl_summarize_static_line_of_sight(&chase_summary_model, 10, 10, 0,
                                            &synthetic_static_los) == 0);
    CHECK(wl_summarize_static_line_of_sight(NULL, 10, 10, 0,
                                            &synthetic_static_los) == -1);
    CHECK(wl_summarize_static_line_of_sight(&chase_summary_model, WL_MAP_SIDE, 10, 0,
                                            &synthetic_static_los) == -1);
    CHECK(wl_summarize_static_line_of_sight(&chase_summary_model, 10, 10, 0, NULL) == -1);
    CHECK(synthetic_static_los.clear_cardinal_count == 2);
    CHECK(synthetic_static_los.blocked_by_wall_count == 1);
    CHECK(synthetic_static_los.blocked_by_door_count == 1);
    CHECK(synthetic_static_los.same_tile_count == 1);
    CHECK(synthetic_static_los.noncardinal_count == 1);
    CHECK(synthetic_static_los.inactive_count == 0);
    CHECK(synthetic_static_los.invalid_position_count == 1);
    CHECK(wl_summarize_static_line_of_sight(&chase_summary_model, 10, 10, 1,
                                            &synthetic_static_los) == 0);
    CHECK(synthetic_static_los.clear_cardinal_count == 1);
    CHECK(synthetic_static_los.inactive_count == 1);
    CHECK(synthetic_static_los.invalid_position_count == 1);
    ...
    chase_summary_model.doors[0].action = WL_DOOR_CLOSED;
    chase_summary_model.doors[0].position = 0;
    chase_summary_model.doors[1].vertical = 0;
    chase_summary_model.doors[1].lock = 1;
    chase_summary_model.doors[1].action = WL_DOOR_OPENING;
    chase_summary_model.doors[1].position = 0x2000;
    chase_summary_model.doors[2].vertical = 1;
    chase_summary_model.doors[2].lock = 2;
    chase_summary_model.doors[2].action = WL_DOOR_CLOSING;
    chase_summary_model.doors[2].position = 0xffff;
    chase_summary_model.doors[3].vertical = 0;
    chase_summary_model.doors[3].lock = 0;
    chase_summary_model.doors[3].action = WL_DOOR_OPEN;
    chase_summary_model.doors[3].position = 0xffff;
    chase_summary_model.doors[4].vertical = 1;
    chase_summary_model.doors[4].lock = 0;
    chase_summary_model.doors[4].action = (wl_door_action)99;
    chase_summary_model.doors[4].position = 0x0100;
    wl_door_state_summary synthetic_door_states;
    CHECK(wl_summarize_door_states(&chase_summary_model, &synthetic_door_states) == 0);
    CHECK(wl_summarize_door_states(NULL, &synthetic_door_states) == -1);
    CHECK(wl_summarize_door_states(&chase_summary_model, NULL) == -1);
    CHECK(synthetic_door_states.vertical_count == 3);
    CHECK(synthetic_door_states.horizontal_count == 2);
    CHECK(synthetic_door_states.unlocked_count == 3);
    CHECK(synthetic_door_states.locked_count == 2);
    CHECK(synthetic_door_states.open_count == 1);
    CHECK(synthetic_door_states.closed_count == 1);
    CHECK(synthetic_door_states.opening_count == 1);
    CHECK(synthetic_door_states.closing_count == 1);
    CHECK(synthetic_door_states.moving_count == 2);
    CHECK(synthetic_door_states.partially_open_count == 2);
    ...
    chase_summary_model.doors[0].x = 10;
    chase_summary_model.doors[0].y = 10;
    chase_summary_model.doors[1].x = 14;
    chase_summary_model.doors[1].y = 10;
    chase_summary_model.doors[2].x = 10;
    chase_summary_model.doors[2].y = 14;
    chase_summary_model.doors[3].x = 6;
    chase_summary_model.doors[3].y = 10;
    chase_summary_model.doors[4].x = 13;
    chase_summary_model.doors[4].y = 13;
    chase_summary_model.doors[5].x = WL_MAP_SIDE;
    chase_summary_model.doors[5].y = 1;
    chase_summary_model.tilemap[10 * WL_MAP_SIDE + 12] = 1;
    chase_summary_model.tilemap[12 * WL_MAP_SIDE + 10] = 0x80u;
    wl_door_line_of_sight_summary synthetic_door_los;
    CHECK(wl_summarize_door_line_of_sight(&chase_summary_model, 10, 10,
                                           &synthetic_door_los) == 0);
    CHECK(wl_summarize_door_line_of_sight(NULL, 10, 10,
                                           &synthetic_door_los) == -1);
    CHECK(wl_summarize_door_line_of_sight(&chase_summary_model, WL_MAP_SIDE, 10,
                                           &synthetic_door_los) == -1);
    CHECK(wl_summarize_door_line_of_sight(&chase_summary_model, 10, 10, NULL) == -1);
    CHECK(synthetic_door_los.clear_cardinal_count == 1);
    CHECK(synthetic_door_los.blocked_by_wall_count == 1);
    CHECK(synthetic_door_los.blocked_by_door_count == 1);
    CHECK(synthetic_door_los.same_tile_count == 1);
    CHECK(synthetic_door_los.noncardinal_count == 1);
    CHECK(synthetic_door_los.invalid_position_count == 1);
    ...
    chase_summary_model.pushwalls[0].dir = WL_DIR_NORTH;
    chase_summary_model.pushwalls[1].x = 3;
    chase_summary_model.pushwalls[1].y = 2;
    chase_summary_model.pushwalls[1].dir = WL_DIR_EAST;
    chase_summary_model.pushwalls[2].x = 4;
    chase_summary_model.pushwalls[2].y = 2;
    chase_summary_model.pushwalls[2].dir = WL_DIR_SOUTH;
    chase_summary_model.pushwalls[3].x = 5;
    chase_summary_model.pushwalls[3].y = 2;
    chase_summary_model.pushwalls[3].dir = WL_DIR_WEST;
    chase_summary_model.pushwalls[4].x = WL_MAP_SIDE;
    chase_summary_model.pushwalls[4].y = 6;
    chase_summary_model.pushwalls[4].dir = WL_DIR_NONE;
    chase_summary_model.pushwall_motion.active = 1;
    chase_summary_model.pushwall_motion.marker_index = 1;
    chase_summary_model.pushwall_motion.x = 3;
    chase_summary_model.pushwall_motion.y = 2;
    chase_summary_model.pushwall_motion.tile = 37;
    chase_summary_model.pushwall_motion.state = 128;
    chase_summary_model.pushwall_motion.pos = 32;
    wl_pushwall_state_summary synthetic_pushwalls;
    CHECK(wl_summarize_pushwall_state(&chase_summary_model, &synthetic_pushwalls) == 0);
    CHECK(wl_summarize_pushwall_state(NULL, &synthetic_pushwalls) == -1);
    CHECK(wl_summarize_pushwall_state(&chase_summary_model, NULL) == -1);
    CHECK(synthetic_pushwalls.marker_count == 5);
    CHECK(synthetic_pushwalls.invalid_marker_position_count == 1);
    CHECK(synthetic_pushwalls.north_count == 1);
    CHECK(synthetic_pushwalls.east_count == 1);
    CHECK(synthetic_pushwalls.south_count == 1);
    CHECK(synthetic_pushwalls.west_count == 1);
    CHECK(synthetic_pushwalls.invalid_direction_count == 1);
    CHECK(synthetic_pushwalls.motion_active == 1);
    CHECK(synthetic_pushwalls.motion_marker_valid == 1);
    CHECK(synthetic_pushwalls.motion_position_valid == 1);
    CHECK(synthetic_pushwalls.motion_tile == 37);
    CHECK(synthetic_pushwalls.motion_state == 128);
    CHECK(synthetic_pushwalls.motion_pos == 32);

    chase_summary_model.pushwall_motion.marker_index = 99;
    chase_summary_model.pushwall_motion.x = WL_MAP_SIDE;
    CHECK(wl_summarize_pushwall_state(&chase_summary_model, &synthetic_pushwalls) == 0);
    ...
    chase_summary_model.path_markers[0].x = 10;
    chase_summary_model.path_markers[0].y = 10;
    chase_summary_model.path_markers[0].dir = WL_DIR_EAST;
    chase_summary_model.path_markers[1].x = 20;
    chase_summary_model.path_markers[1].y = 20;
    chase_summary_model.path_markers[1].dir = WL_DIR_SOUTH;
    chase_summary_model.tilemap[20 * WL_MAP_SIDE + 20 + WL_MAP_SIDE] = 1;
    chase_summary_model.path_markers[2].x = 0;
    chase_summary_model.path_markers[2].y = 0;
    chase_summary_model.path_markers[2].dir = WL_DIR_WEST;
    chase_summary_model.path_markers[3].x = 30;
    chase_summary_model.path_markers[3].y = 30;
    chase_summary_model.path_markers[3].dir = WL_DIR_NONE;
    chase_summary_model.path_markers[4].x = 31;
    chase_summary_model.path_markers[4].y = 31;
    chase_summary_model.path_markers[4].dir = (wl_direction)99;
    chase_summary_model.path_markers[5].x = WL_MAP_SIDE;
    chase_summary_model.path_markers[5].y = 1;
    chase_summary_model.path_markers[5].dir = WL_DIR_NORTH;
    wl_path_marker_exit_summary synthetic_path_exits;
    CHECK(wl_summarize_path_marker_exits(&chase_summary_model,
                                          &synthetic_path_exits) == 0);
    CHECK(wl_summarize_path_marker_exits(NULL, &synthetic_path_exits) == -1);
    CHECK(wl_summarize_path_marker_exits(&chase_summary_model, NULL) == -1);
    CHECK(synthetic_path_exits.open_exit_count == 1);
    CHECK(synthetic_path_exits.wall_blocked_exit_count == 1);
    CHECK(synthetic_path_exits.out_of_bounds_exit_count == 1);
    CHECK(synthetic_path_exits.no_direction_count == 1);
    ...
    CHECK(wl_clear_present_frame_rgba_padding(&present_frame,
                                              ...
                                              0x7e) == 0);
    memset(present_rgba_padded, 0xa5, sizeof(present_rgba_padded));
    CHECK(wl_expand_present_frame_to_rgba_pitched_clear_padding(
              &present_frame, present_rgba_padded, sizeof(present_rgba_padded),
              PRESENT_PADDED_PITCH, 0x7e, &present_rgba_padded_upload) == 0);
    CHECK(present_rgba_padded_upload.format == WL_TEXTURE_UPLOAD_RGBA8888);
    CHECK(present_rgba_padded_upload.pitch == PRESENT_PADDED_PITCH);
    CHECK(present_rgba_padded_upload.pixel_bytes == sizeof(present_rgba_padded));
    for (size_t row = 0; row < 128u; ++row) {
        CHECK(memcmp(present_rgba_padded + row * PRESENT_PADDED_PITCH,
                     present_rgba + row * 80u * 4u, 80u * 4u) == 0);
        for (size_t pad = 80u * 4u; pad < PRESENT_PADDED_PITCH; ++pad) {
            CHECK(present_rgba_padded[row * PRESENT_PADDED_PITCH + pad] == 0x7e);
        }
    }
    CHECK(wl_expand_present_frame_to_rgba_pitched_clear_padding(
              &present_frame, present_rgba_padded, sizeof(present_rgba_padded) - 1u,
              PRESENT_PADDED_PITCH, 0x7e, &present_rgba_padded_upload) == -1);
    CHECK(wl_expand_present_frame_to_rgba_pitched_clear_padding(
              NULL, present_rgba_padded, sizeof(present_rgba_padded),
              PRESENT_PADDED_PITCH, 0x7e, &present_rgba_padded_upload) == -1);
    CHECK(wl_clear_present_frame_rgba_padding(&present_frame,
                                              present_rgba_padded,
...
static int check_audio_wl6(const char *dir) {
    ...
    {
        uint32_t saved_offset = audio.offsets[0];
        uint32_t saved_next = audio.offsets[1];
        audio.offsets[0] = (uint32_t)audiot_size + 1u;
        audio.offsets[1] = audio.offsets[0];
        CHECK(wl_validate_audio_header_offsets(&audio, audiot_size) == -1);
        CHECK(wl_read_audio_chunk(audiot_path, &audio, 0,
                                  chunk_buf, sizeof(chunk_buf), &chunk_bytes) == -1);
        audio.offsets[0] = saved_offset;
        audio.offsets[1] = saved_next;
    }
    {
