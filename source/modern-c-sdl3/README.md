...
The first runnable milestone is a pure C asset/decompression/semantics/model harness. It has no SDL dependency yet and verifies local WL6 data discovery, `MAPHEAD`, `GAMEMAPS`, `VGAHEAD`/`VGADICT`/`VGAGRAPH` Huffman graphics smoke decoding, full `VSWAP` chunk-directory parsing, bounded chunk reads, wall/sprite shape metadata, sprite post-command metadata, Carmack/RLEW map-plane decompression, first-pass map semantic classification, and a minimal `SetupGameLevel`-style runtime model.
