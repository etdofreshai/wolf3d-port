...
- Added `wl_summarize_runtime_player_interaction`, a combined player-facing preflight that reports the nearest visible actor/static candidate plus first map blocker metadata. The summary now also carries ahead/blocked actor and static counts so future use/combat/HUD prompts can distinguish an empty sightline from map-blocked candidates with one SDL-free query and no live-state mutation.
