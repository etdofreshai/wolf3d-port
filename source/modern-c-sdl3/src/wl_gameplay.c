...
int wl_use_player_facing(wl_player_gameplay_state *state, wl_game_model *model,
    ...
    if (!button_held && (doornum & 0x80u)) {
        ...
        if (lock >= 1u && lock <= 4u) {
            ...
            out->has_required_key = (state->keys & (1u << out->required_key)) ? 1u : 0u;
            if (!out->has_required_key) {
                out->locked = 1;
                return 0;
            }
        }
...
static int step_live_actor_ai_tick_impl(wl_player_gameplay_state *state,
                                        wl_game_model *model,
                                        const uint16_t *wall_plane,
                                        const uint16_t *info_plane,
                                        size_t word_count,
                                        ...
                                        int32_t forward_x, int32_t forward_y,
                                        wl_direction facing, int use_button,
                                        int button_held, int wake_before_step,
                                        int include_ambush, int search_forward,
    ...
    memset(out, 0, sizeof(*out));
    out->wake.first_woken_actor = UINT16_MAX;
    out->wake.last_woken_actor = UINT16_MAX;
    if (wl_step_live_tick(state, model, wall_plane, info_plane, word_count,
                          motion, xmove, ymove, forward_x, forward_y, facing,
                          use_button, button_held, tics, &out->live) != 0) {
        return -1;
    ...
    if (wake_before_step) {
        if (wl_wake_actors_for_chase(model, motion->tile_x, motion->tile_y,
                                     include_ambush, search_forward,
                                     &out->wake) != 0) {
            return -1;
        }
        out->actors_woke = out->wake.actors_woke > 0 ? 1u : 0u;
    }
...
int wl_step_live_actor_ai_tick(wl_player_gameplay_state *state,
                               ...
                               int32_t tics,
                               wl_live_actor_ai_tick_result *out) {
    return step_live_actor_ai_tick_impl(state, model, wall_plane, info_plane,
                                        word_count, motion, xmove, ymove,
                                        forward_x, forward_y, facing,
                                        use_button, button_held, 0, 0, 1,
                                        patrol_speed, tics, out);
}
