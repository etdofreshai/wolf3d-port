...
int wl_clear_present_frame_rgba_padding(const wl_present_frame_descriptor *present,
    ...
    for (size_t y = 0; y < height; ++y) {
        memset(rgba + y * rgba_pitch + tight_pitch, value, padding_per_row);
    }
    return 0;
}
...
int wl_schedule_advance_sound_channel_from_chunk(
    ...
    out->schedule = schedule;
    if (schedule.held) {
        out->advance.state = schedule.state;
        return 0;
    }
    if (wl_advance_sound_channel_from_chunk(&schedule.state, metadata, chunk,
                                            chunk_size, sample_delta, &advance) != 0) {
        return -1;
    }
    out->advance = advance;
    out->advanced = 1u;
...
int wl_schedule_describe_sound_channel_window_from_chunk(
    ...
    if (wl_describe_sound_playback_window_from_chunk(metadata, chunk, chunk_size,
                                                     schedule.state.sample_position,
                                                     sample_budget, &window) != 0) {
        return -1;
    }
    out->window = window;
    out->described = 1u;
...
int wl_schedule_describe_sound_channel_position_from_chunk(
    ...
    if (wl_describe_sound_playback_position_from_chunk(metadata, chunk, chunk_size,
                                                       schedule.state.sample_position,
                                                       &position) != 0) {
        return -1;
    }
...
int wl_schedule_describe_sound_channel_progress_from_chunk(
    ...
    if (wl_describe_sound_channel_progress_from_chunk(&schedule.state, metadata,
                                                      chunk, chunk_size,
                                                      &progress) != 0) {
        return -1;
    }
...
int wl_schedule_describe_sound_channel_progress_window_from_chunk(
    const wl_sound_channel_state *current,
    size_t candidate_chunk,
    const wl_audio_chunk_metadata *metadata,
    const unsigned char *chunk, size_t chunk_size,
    size_t sample_budget,
    wl_sound_channel_schedule_progress_window_result *out) {
    wl_sound_channel_schedule_result schedule;
    ...
    if (wl_describe_sound_channel_progress_window_from_chunk(&schedule.state, metadata,
                                                             chunk, chunk_size, sample_budget,
                                                             &progress_window) != 0) {
        return -1;
    }
...
int wl_describe_sound_channel_progress_from_chunk(const wl_sound_channel_state *current,
                                                  const wl_audio_chunk_metadata *metadata,
                                                  const unsigned char *chunk, size_t chunk_size,
                                                  wl_sound_channel_progress *out) {
    wl_sample_playback_position position;
    if (!current || !metadata || !out || current->active != 1u) {
        return -1;
    }
    if (wl_describe_sound_playback_position_from_chunk(metadata, chunk, chunk_size,
    ...
    memset(out, 0, sizeof(*out));
    out->sound_index = current->sound_index;
    out->priority = current->priority;
    out->kind = metadata->kind;
    out->sample_position = position.sample_position;
    out->sample_count = position.sample_count;
    out->remaining_samples = position.sample_count - position.sample_position;
    out->current_sample = position.current_sample;
    out->completed = position.completed;
    return 0;
}

int wl_describe_sound_channel_progress_window_from_chunk(
    const wl_sound_channel_state *current,
    const wl_audio_chunk_metadata *metadata,
    const unsigned char *chunk, size_t chunk_size,
    size_t sample_budget,
    wl_sound_channel_progress_window *out) {
    if (!out) {
        return -1;
    }
    if (wl_describe_sound_channel_progress_from_chunk(current, metadata, chunk, chunk_size,
                                                      &out->progress) != 0) {
        return -1;
    }
    if (wl_describe_sound_channel_window_from_chunk(current, metadata, chunk, chunk_size,
                                                    sample_budget, &out->window) != 0) {
        memset(out, 0, sizeof(*out));
        return -1;
    }
    return 0;
...
int wl_describe_sound_channel_window_from_chunk(const wl_sound_channel_state *current,
    ...
    return wl_describe_sound_playback_window_from_chunk(metadata, chunk, chunk_size,
                                                        current->sample_position,
                                                        sample_budget, out);
}
