...
int wl_summarize_path_marker_endpoints(
    ...
    uint16_t incoming_counts[WL_MAX_PATH_MARKERS];
    memset(incoming_counts, 0, sizeof(incoming_counts));
    memset(out, 0, sizeof(*out));
    ...
    for (size_t i = 0; i < model->path_marker_count; ++i) {
        ...
        ++out->linked_exit_count;
        if (incoming_counts[next_index] != UINT16_MAX) {
            ++incoming_counts[next_index];
        }
    }
    ...
    for (size_t i = 0; i < model->path_marker_count; ++i) {
        ...
        } else if (incoming_counts[i] > 1) {
            ++out->branch_in_marker_count;
            if (out->first_branch_in_marker_index == UINT16_MAX) {
                out->first_branch_in_marker_index = (uint16_t)i;
            }
        }
...
int wl_summarize_runtime_tiles(const wl_game_model *model,
    ...
    memset(out, 0, sizeof(*out));
    out->first_solid_wall_tile = UINT16_MAX;
    for (size_t y = 0; y < WL_MAP_SIDE; ++y) {
        for (size_t x = 0; x < WL_MAP_SIDE; ++x) {
            ...
            if (tile == 0u) {
                ...
                ++out->moving_pushwall_marker_count;
            } else if ((tile & 0x80u) != 0u) {
                ++out->door_marker_count;
            } else if (tile <= 63u) {
                ++out->solid_wall_count;
                if (tile < out->first_solid_wall_tile) {
                    out->first_solid_wall_tile = tile;
                }
                if (tile > out->last_solid_wall_tile) {
                    out->last_solid_wall_tile = tile;
                }
            } else {
    ...
    if (out->solid_wall_count == 0u) {
        out->first_solid_wall_tile = 0u;
    }
    return 0;
}
...
int wl_summarize_runtime_player_facing_tile(
    ...
    const int tx = (int)model->player.x + dx;
    const int ty = (int)model->player.y + dy;
    if (tx < 0 || ty < 0 || tx >= (int)WL_MAP_SIDE || ty >= (int)WL_MAP_SIDE) {
        return 0;
    }

    out->has_tile = 1u;
    out->tile_x = (uint16_t)tx;
    out->tile_y = (uint16_t)ty;
    out->tile = model->tilemap[map_index((size_t)tx, (size_t)ty)];
    if (out->tile == 0u) {
        out->clear_floor = 1u;
    } else if ((out->tile & 0xc0u) == 0xc0u) {
        out->pushwall_marker = 1u;
    } else if ((out->tile & 0x80u) != 0u) {
        out->door_marker = 1u;
    } else if (out->tile <= 63u) {
        out->solid_wall = 1u;
    } else {
        out->other_marker = 1u;
    }
...
int wl_summarize_actor_attack_readiness(const wl_game_model *model,
    ...
    for (size_t i = 0; i < model->actor_count; ++i) {
        ...
        if (blocked) {
            ...
            if (actor->tile_y == player_y) {
                ++out->ready_horizontal_sight_count;
            } else {
                ++out->ready_vertical_sight_count;
            }
