...
- `wl_schedule_describe_sound_channel_progress_window_from_chunk()` adds the scheduling-facing variant: accepted sounds expose both reset progress metadata and the first bounded sample window, while held lower-priority sounds preserve the active channel and intentionally leave the progress/window descriptor absent.
