...
- `wl_schedule_describe_sound_channel_position_from_chunk()` combines metadata-backed scheduling with reset-position inspection so presentation/mixer code can learn the first sample/count for accepted sounds without advancing playback or touching held candidates.
- `wl_schedule_describe_sound_channel_progress_from_chunk()` combines metadata-backed scheduling with the compact progress descriptor: accepted sounds expose reset sample/count/remaining/current-sample metadata, while held lower-priority candidates preserve the active channel without inspecting unrelated candidate bytes.
...
- `wl_describe_sound_channel_progress_from_chunk()` returns a compact active-channel progress descriptor combining chunk kind, sound index, priority, sample position/count, remaining samples, current sample, and completion state for future mixer/debug UI seams.
- `wl_describe_sound_channel_progress_window_from_chunk()` combines that active-channel progress descriptor with the next bounded playback window so future mixer/debug code can report status and buffer contents through one validated metadata-backed seam.
- `wl_schedule_describe_sound_channel_progress_window_from_chunk()` combines schedule arbitration with the progress-window descriptor: accepted sounds expose reset progress metadata plus the first bounded emission window, while held lower-priority candidates preserve the active channel and intentionally leave the progress/window descriptor absent.
- `wl_schedule_tick_describe_sound_channel_progress_window_from_chunk()` combines scheduling, a bounded sample tick, and post-tick progress/window inspection for mixer loops that need to emit a buffer and immediately describe the next buffered span.
