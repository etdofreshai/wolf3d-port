...
- `wl_tick_sound_channel()` advances an active PC speaker or AdLib channel through the existing bounded sample cursor helpers, preserves inactive channels, and clears `active` when playback reaches the declared sample end.
