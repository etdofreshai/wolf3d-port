...
`wl_step_live_player_fire_tick()` now uses the attack-start helper when the fire button is active, preserving the existing `fire` metadata while also reporting `fire_attack` frame-start metadata. This keeps the live SDL-free tick path aligned with the player attack-state seam: a successful shot consumes ammo and starts the weapon-specific attack frame, no-ammo fallback preserves the current frame after the regular live tick advance, and idle ticks leave fire/attack-start descriptors empty.
