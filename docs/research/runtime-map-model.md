...
Added `wl_summarize_actor_modes()` as the exact-mode sibling to the higher-level activity summary. It reports stand/patrol/chase/inert/boss/ghost mode buckets and invalid mode/position counts, keeping runtime actor state drift visible without mutating actor descriptors.
...
`wl_summarize_runtime_tile_diagonals` adds a small live-tilemap diagonal diagnostic, partitioning the two corner-to-corner diagonals into solid walls, clear floor, and runtime markers. The marker totals now retain door, moving-pushwall, and other marker sub-buckets, giving spawn/camera/raycast smoke work a cheap headless blocker-class guard without requiring callers to rescan the tilemap.
`wl_summarize_runtime_tile_axes` adds the matching center row/column diagnostic, splitting each axis into solid wall, clear floor, and marker buckets with an explicit center-tile overlap count. Marker totals are also sub-bucketed into door, moving-pushwall, and other runtime markers so crosshair-axis diagnostics can distinguish live blocker classes without a separate tilemap scan. Real-map and synthetic tests pin the totals for future spawn/raycast/minimap axis checks.
`wl_summarize_runtime_tile_corners` adds a focused four-corner diagnostic for live tilemaps, preserving the raw corner tile ids while bucketizing wall, floor, door-marker, pushwall-marker, and other-marker cases. Real WL6 coverage ties the raw values to the loaded runtime tilemap, and synthetic coverage pins all non-floor buckets for boundary-integrity checks.
