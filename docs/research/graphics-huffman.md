...
Added a small `wl_gameplay` seam that connects original-style player events to the palette-shift state without depending on the DOS status bar or VGA hardware. `wl_apply_player_damage` mirrors `TakeDamage` behavior for victory no-op, baby-difficulty quarter damage, god-mode health preservation, death state, `gotgatgun` reset, and `StartDamageFlash` damage accumulation. `wl_start_player_bonus_flash` routes bonus events to the white-flash counter. `wl_award_player_points` mirrors `GivePoints` score/extra-life threshold progression using `EXTRAPOINTS=40000`, including advancing thresholds even when lives are capped at 9. `wl_summarize_player_resources` snapshots health, ammo, keys, weapons, attack activity, and active flash counters for a future status-bar/HUD presentation seam without coupling it to SDL. Headless tests assert the resulting health, score, lives, thresholds, resources, and palette-shift counters.
